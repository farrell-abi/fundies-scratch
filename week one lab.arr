...
#|T-Shirt Shop
   Note: for the sake of my sanity I will be using the American dollar sign ($) because I am too lazy to find the pound symbol and type it every time. Thank you in advance for using your imagination and pretending it is the pound sign. :)
...
#| The expressions are virtually the same. The second expression can also be written as {(7 * 12) + 3} so that the expressions look even closer. The only reason I used a variable initially for the first expression is for simplicity and, thankfully, the understanding of basic multiplication.|#

#| Rectangular Poster price of poster is based on the perimeter * $0.10|#

#Perimeter of 420mm X 594mm
p = 2 * (420 + 594)
p
...
#| Forgetting the parentheses in the perimeter equation would cause an error because the two operations are 'at the same grouping level' (I do not know what this means, that is what the error message told me), which causes the system to have difficulty deciding the order of operations.|#
...
#rectangle(50, "solid", 20, "black") -> this creates an error
#the fix is to switch the order of the inputs to (width, height, solid/outline, color)
rectangle(50, 20, 'solid', 'black')

#circle(30, solid, 'red') -> this creates an error
